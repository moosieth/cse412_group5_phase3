...
firebase_app = firebase_admin.initialize_app(
    cred, {"storageBucket": "group5-inql.appspot.com"}
)
...
def login():
    ...
    if bcrypt.checkpw(byte_password, stored_password.encode()):
        cursor.execute(
            f'SELECT userID FROM User WHERE email="{email}" AND pw="{stored_password}"'
        )
...
def searchalbum():
    ...
    cursor.execute(
        f'SELECT albumID, name FROM Album WHERE name LIKE "%{aName}%" AND userID = {uid}'
    )
    tuples = cursor.fetchall()
...
def add():
    try:
        ...
        print(request.json)
        target = (
            request.form.get("target") if request.form else request.json.get("target")
        )
        if target is None:
        ...
        if target == "photo":
            ...
            newEntry = request.json
            query = (
                f"INSERT INTO Likes VALUES({newEntry['userID']}, {newEntry['photoID']})"
            )
        elif target == "comment":
            newEntry = request.json
...
def photobytag():
    ...
    for i, name in enumerate(allNames):
        subQueries[
            i
        ] = f'SELECT Tag.photoID, COUNT(*) AS num FROM Photo JOIN Tag ON Photo.photoID = Tag.photoID WHERE Tag.name LIKE "%{allNames[i]}%" GROUP BY Tag.photoID'
        print(subQueries[i])
...
def myphotobytag():
    ...
    query1 = f"CREATE VIEW {uid}Photos AS (SELECT Photo.photoID AS pid, Photo.caption AS cap, Photo.data AS dat, Photo.albumID as aid FROM Photo INNER JOIN Album ON Photo.albumID = Album.albumID WHERE Album.userID = {uid})"
    ...
    for i, name in enumerate(allNames):
        subQueries[
            i
        ] = f'SELECT Tag.photoID, COUNT(*) AS num FROM {uid}Photos JOIN Tag ON {uid}Photos.pid = Tag.photoID WHERE Tag.name LIKE "%{allNames[i]}%" GROUP BY Tag.photoID'
        print(subQueries[i])

    masterQuery = f"SELECT total_matches.photoID, {uid}Photos.cap, {uid}Photos.dat, SUM(num), Album.userID FROM ("
    ...
    masterQuery += f") AS total_matches JOIN ({uid}Photos JOIN Album ON {uid}Photos.aid = Album.albumID) ON total_matches.photoID = {uid}Photos.pid GROUP BY total_matches.photoID ORDER BY SUM(num) DESC"
...
def trendingtags():
    ...
    cursor.execute(
        "SELECT name, COUNT(name) FROM Tag GROUP BY name ORDER BY COUNT(name) DESC LIMIT 10"
    )
    tuples = cursor.fetchall()
        ...
        for t in tuples:
            if t[3] is not None:
                ...
                try:
                    ...
                    firebase_url = firebase_storage.generate_signed_url(
